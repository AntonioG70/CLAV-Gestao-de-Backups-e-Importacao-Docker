...
async function writeFile(c, dados, token, flag, path){
  ...
  if(flag){
    for(const rada of dados.data){
      ...
      let entRada = rada.ent.split("#")[1]
      let fileName = 'Despacho_' + rada.numero + '_RADA_' + entRada 
      await axios.get('http://clav-api.di.uminho.pt/v2/rada/old/' + radaID + '?apikey=' + token)
        .then(async p => {
          ...
          fs.appendFileSync(path + "/manifest-sha256.txt", sum + " data/radaOLD/" + fileName + '.json\n')      
        })
