...
async function total(obj, token){
  for(const element of obj) {
    let query = converter.legislacaoToTtl([element])

    await axios({
      method: 'post',
      url: "http://clav-api.di.uminho.pt/v2/legislacao/repor?token=" + token,
...
async function add(obj, token){
  for(const element of obj){
    await axios.get('http://clav-api.di.uminho.pt/v2/legislacao/' + element.id + '?token=' + token)
      .then(dados => {
...
async function overwrite(obj, token){
  await axios.get('http://clav-api.di.uminho.pt/v2/legislacao?token=' + token)
  .then(async dados => {
    for(const l of dados.data){
      await axios.delete('http://clav-api.di.uminho.pt/v2/legislacao/' + l.id  + '?token=' + token)
        .then(async () => {
          console.log(l.id + ' eliminada')
        })
        .catch(err => {
